...
async function getRecentOrders() {
  try {
    const supabase = createAdminClient();
    const { data } = await supabase.from("orders").select("id, first_name, last_name, total, status, payment_method, created_at").order("created_at", { ascending: false }).limit(5);
    return data ?? [];
  } catch { return []; }
}
...
export default async function AdminDashboard() {
  ...
  const cards = [
    ...
    { label: "Revenue", value: formatPrice(stats.totalRevenue), sub: "excl. cancelled/refunded" },
    ...
    <div>
      ...
      <div className="rounded-[16px] overflow-hidden" style={{ border: "1px solid var(--line)", background: "var(--surface)" }}>
        ...
        <div className="divide-y" style={{ borderColor: "var(--line)" }}>
          ...
          {recent.map((o: any) => (
            <Link key={o.id} href={`/admin/orders/${o.id}`} className="flex items-center justify-between px-5 py-4 hover:bg-white/[.02] transition-colors">
              <div>
              ...
              <div className="text-right">
                <p className="text-sm font-bold" style={{ fontFamily: "var(--font-hanken)" }}>{formatPrice(o.total)}</p>
                <span className="text-[.65rem] font-bold px-2 py-0.5 rounded-full uppercase" style={{ background: `${STATUS_COLORS[o.status] ?? "#9ca3af"}22`, color: STATUS_COLORS[o.status] ?? "#9ca3af", fontFamily: "var(--font-space-mono)" }}>
                  {o.status}
                </span>
              </div>
            </Link>
